...
const AttendanceRecapSystem = () => {
  ...
  const clearAllHolidays = () => {
    ...
    if (window.confirm('Apakah Anda yakin ingin menghapus SEMUA daftar hari libur?')) {
      setHolidays([]);
      localStorage.setItem('holidays', JSON.stringify([]));
    }
  };

  // State untuk Input Mode Libur
  const [holidayInputMode, setHolidayInputMode] = useState('single'); // 'single' | 'range'
  ...
  const addHolidayRange = () => {
    ...
    if (newDates.length === 0) {
      alert('Semua tanggal dalam rentang ini sudah ada di daftar libur!');
      return;
    }
    ...
    setHolidayStartDate('');
    setHolidayEndDate('');
    alert(`Berhasil menambahkan ${newDates.length} hari libur!`);
  };
  ...
  const generateRecapTables = () => {
    ...
    try {
      ...
      let totalHariKerja = 0; // Total akumulasi hari kerja semua karyawan
      let totalHadir = 0;
      let totalTepat = 0;
      let totalTelat = 0;
      let totalAlfa = 0;

      // Hitung Hari Kerja Efektif (Hari dimana sekolah masuk / ada aktifitas)
      // Logika: Hari kerja adalah hari dimana TIDAK SEMUA karyawan libur (L)
      let effectiveHariKerja = 0;
      dateRange.forEach(dateStr => {
        // Cek jika ada setidaknya satu karyawan yang statsnya BUKAN 'L' pada tanggal ini
        const isWorkingDay = recap.some(emp => emp.dailyEvaluation[dateStr].text !== 'L');
        if (isWorkingDay) {
          effectiveHariKerja++;
        }
      });
      ...
      setSummaryData({
        ...
        totalHariKerja, // Total akumulasi (Column 4)
        effectiveHariKerja, // Hari Kerja Sekolah (Column 3)
        totalHadir,
        persentaseTepat,
        totalTepat,
        persentaseTelat,
  ...
  const generateSummary = () => {
    ...
    // Hitung effectiveHariKerja untuk generateSummary
    let effectiveHariKerja = 0;
    recapData.dateRange.forEach(dateStr => {
      const isWorkingDay = recapData.recap.some(emp => emp.dailyEvaluation[dateStr].text !== 'L');
      if (isWorkingDay) {
        effectiveHariKerja++;
      }
    });
    ...
    setSummaryData({
      ...
      persentaseAlfa,
      totalAlfa,
      effectiveHariKerja, // Pastikan ini juga diupdate saat generateSummary dipanggil manual
      periode,
    });
  ...
  const downloadCompletePdf = async () => {
    try {
      ...
      // 5 KOTAK INFORMASI UTAMA (1 BARIS)
      const summaryBoxes = [
        {
          label: 'Pra-Kesimpulan', // Label di atas box
          value: summaryData.predikat,
          title: 'PREDIKAT', // Masuk ke dalam box sebagai title kecil
          color: [240, 240, 255],
          fontSize: 14,
        },
        {
        ...
        {
          label: 'Hari Kerja',
          value: `${summaryData.effectiveHariKerja || 0} hari`, // NEW
          subLabel: '(Selain Libur)',
          color: [230, 230, 240],
          fontSize: 16,
        },
        {
          label: 'Total Hari Kerja G&K',
      ...
      // Bagi 5 kolom
      const summaryBoxWidth = (pageWidth - 80 - (4 * summarySpacing)) / 5;
      ...
      summaryBoxes.forEach((box, idx) => {
        ...
        // Handle Predikat has specific label logic in array? No, simpler to just start printing
        // Print Label
        if (box.title) {
          doc.text(box.title, xPos + 10, yPos + 20);
        } else {
          // Split label if too long
          const splitLabel = doc.splitTextToSize(box.label, summaryBoxWidth - 20);
          doc.text(splitLabel, xPos + 10, yPos + 20);
        }

        // Value
        doc.setFontSize(box.fontSize);
        doc.setFont(undefined, 'bold');
        doc.text(box.value, xPos + 10, yPos + 45);

        // Sublabel if exists
        if (box.subLabel) {
          doc.setFontSize(7);
          doc.setFont(undefined, 'italic');
          doc.text(box.subLabel, xPos + 10, yPos + 58);
        }
      });
  ...
  const downloadSummaryAsPdf = () => {
    ...
    // Kita punya 3 box sekarang: Total Karyawan | Hari Kerja | Total Hari Kerja G&K
    const infoBoxWidth = (pageWidth - 100) / 3;
    const infoY = yPos;
    const infoHeight = 70;

    // Box 1: Total Karyawan
    doc.setFillColor(230, 230, 240);
    doc.roundedRect(40, infoY, infoBoxWidth, infoHeight, 5, 5, 'F');
    doc.setFont(undefined, 'normal');
    doc.setFontSize(10);
    doc.text('Total Karyawan', 50, infoY + 20);
    doc.setFont(undefined, 'bold');
    doc.setFontSize(16);
    doc.text(`${summaryData.totalKaryawan} orang`, 50, infoY + 45);

    // Box 2: Hari Kerja (New)
    doc.setFillColor(230, 230, 240);
    doc.roundedRect(40 + infoBoxWidth + 10, infoY, infoBoxWidth, infoHeight, 5, 5, 'F');
    doc.setFont(undefined, 'normal');
    doc.setFontSize(10);
    doc.text('Hari Kerja', 50 + infoBoxWidth + 10, infoY + 20);
    doc.setFont(undefined, 'bold');
    doc.setFontSize(16);
    doc.text(`${summaryData.effectiveHariKerja || 0} hari`, 50 + infoBoxWidth + 10, infoY + 45);
    doc.setFontSize(8);
    doc.setFont(undefined, 'italic');
    doc.text('(selain libur)', 50 + infoBoxWidth + 10, infoY + 60);

    // Box 3: Total Hari Kerja G&K
    doc.setFillColor(230, 230, 240);
    doc.roundedRect(40 + 2 * (infoBoxWidth + 10), infoY, infoBoxWidth, infoHeight, 5, 5, 'F');
    doc.setFont(undefined, 'normal');
    doc.setFontSize(10);
    doc.text('Total Hari Kerja G&K', 50 + 2 * (infoBoxWidth + 10), infoY + 20);
    doc.setFont(undefined, 'bold');
    doc.setFontSize(16);
    doc.text(
      `${summaryData.totalHariKerja} hari`,
      50 + 2 * (infoBoxWidth + 10),
      infoY + 45
    );
      ...
      >
        ...
        */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
          ...
          <div className="bg-white bg-opacity-20 rounded-lg p-5 flex flex-col justify-center relative overflow-hidden">
            <p className="text-sm font-semibold mb-1 opacity-90">
              Periode & Predikat
            </p>
            <p className="text-xs opacity-80 mb-2">{summaryData.periode}</p>
            <p className="text-2xl font-bold">{summaryData.predikat}</p>
            <div className="text-sm mt-1 opacity-90">
              {summaryData.persentaseKehadiran}% Kehadiran
            </div>
          </div>
          ...
          {/* KOLOM 3: Hari Kerja (Baru) */}
          <div className="bg-white bg-opacity-20 rounded-lg p-5 flex flex-col justify-center">
            <p className="text-sm opacity-90 mb-1">
              Hari Kerja
            </p>
            <p className="text-3xl font-bold">
              {summaryData.effectiveHariKerja || 0} hari
            </p>
            <p className="text-xs opacity-70 italic mt-1">(selain hari libur)</p>
          </div>

          {/* KOLOM 4: Total Hari Kerja G&K */}
          <div className="bg-white bg-opacity-20 rounded-lg p-5 flex flex-col justify-center">
            <p className="text-sm opacity-90 mb-1">
              Total Hari Kerja G&K
            </p>
            <p className="text-3xl font-bold">
              {summaryData.totalHariKerja} hari
            </p>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
                ...
                <div>
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="flex bg-gray-100 rounded-lg p-0.5">
                      <button
                        onClick={() => setHolidayInputMode('single')}
                        className={`px-2 py-0.5 text-xs rounded-md transition-all ${holidayInputMode === 'single'
                      ...
                      <button
                        onClick={() => setHolidayInputMode('range')}
                        className={`px-2 py-0.5 text-xs rounded-md transition-all ${holidayInputMode === 'range'
                          ? 'bg-white text-indigo-600 shadow-sm font-medium'
                          : 'text-gray-500 hover:text-gray-700'
                          }`}
                      >
                  ...
                  {holidayInputMode === 'single' ? (
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex gap-2 items-center">
                        <div className="flex-1">
                          <input
                            ...
                            className="w-full px-2 py-1 border border-gray-300 rounded text-xs focus:ring-2 focus:ring-indigo-500 outline-none"
                            placeholder="Mulai"
                          />
                        </div>
                      ...
                      <button
                        ...
                        className="w-full bg-indigo-600 text-white py-1.5 rounded-lg hover:bg-indigo-700 transition-colors text-xs font-medium flex items-center justify-center gap-1"
                      >
                        <Plus size={14} /> Tambah Rentang Libur
                      </button>
                    </div>
                  )}
                </div>
          ...
          <div className="space-y-8">
            ...
            {activeRecapTab === 'guide' && (
              <div
                ref={panduanRef}
                className="mt-6 p-4 md:p-6 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl"
              >
                <div className="max-w-full mx-auto">
                  {/* Judul dan Tombol - Responsive */}
                  <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-6">
                    <h4 className="text-xl md:text-2xl font-bold text-gray-800 flex items-center gap-2 order-2 md:order-1">
                  ...
                  {/* TABEL 1: Rekap Mesin */}
                  <div className="mb-6 bg-white p-4 md:p-6 rounded-lg shadow-sm border-l-4 border-blue-500">
                    <h5 className="font-bold text-lg md:text-xl text-blue-700 mb-3">
                    ...
                    <div className="space-y-4 md:grid md:grid-cols-2 md:gap-4">
                      ...
                      <div className="bg-blue-50 p-3 md:p-4 rounded-lg border border-blue-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-blue-300 rounded-lg flex flex-col items-center justify-center text-xs font-bold text-white">
                              <div>07:15</div>
                              <div>16:30</div>
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Scan <strong>MASUK dan PULANG</strong> keduanya tercatat
                            </p>
                      ...
                      <div className="bg-yellow-50 p-3 md:p-4 rounded-lg border border-yellow-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-yellow-300 rounded-lg flex flex-col items-center justify-center text-xs font-bold text-yellow-900">
                              <div>07:15</div>
                              <div>-</div>
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Hanya scan <strong>MASUK</strong> atau <strong>PULANG</strong> saja
                            </p>
                      ...
                      <div className="bg-red-50 p-3 md:p-4 rounded-lg border border-red-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-red-300 rounded-lg flex flex-col items-center justify-center text-xs font-bold text-red-900">
                              <div>-</div>
                              <div>-</div>
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Tidak ada scan <strong>MASUK maupun PULANG</strong> (Alpha)
                            </p>
                      ...
                      {/* Putih L */}
                      <div className="bg-gray-50 p-3 md:p-4 rounded-lg border border-gray-300">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Hari <strong>LIBUR</strong> atau tidak ada jadwal (OFF/Jumat)
                            </p>
                  ...
                  {/* TABEL 2: Kedisiplinan Waktu */}
                  <div className="mb-6 bg-white p-4 md:p-6 rounded-lg shadow-sm border-l-4 border-green-500">
                    <h5 className="font-bold text-lg md:text-xl text-green-700 mb-3">
                      2️⃣ Kedisiplinan Waktu (Evaluasi Disiplin)
                    </h5>
                    <p className="text-sm md:text-base text-gray-700 mb-5">
                      Evaluasi kedisiplinan berdasarkan jadwal mengajar dan kelengkapan scan
                    </p>
                    ...
                    <div className="space-y-4 md:grid md:grid-cols-2 md:gap-4">
                      ...
                      <div className="bg-green-50 p-3 md:p-4 rounded-lg border border-green-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-green-300 rounded-lg flex flex-col items-center justify-center text-xs font-bold text-green-900">
                              <div>J: 07:00</div>
                              <div>S: 06:45</div>
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Datang <strong>SEBELUM</strong> jadwal mengajar dimulai
                            </p>
                      ...
                      <div className="bg-blue-50 p-3 md:p-4 rounded-lg border border-blue-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-blue-300 rounded-lg flex flex-col items-center justify-center text-xs font-bold text-blue-900">
                              <div>07:15</div>
                              <div>16:30</div>
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Scan <strong>MASUK dan PULANG</strong> lengkap (administrasi tertib)
                            </p>
                            <p className="text-xs text-blue-700 mt-1 italic">
                              📋 Tertib administrasi
                            </p>
                      ...
                      <div className="bg-yellow-50 p-3 md:p-4 rounded-lg border border-yellow-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Datang tapi <strong>LUPA scan pulang</strong>, atau datang terlambat
                            </p>
                            <p className="text-xs text-yellow-700 mt-1 italic">
                      ...
                      <div className="bg-red-50 p-3 md:p-4 rounded-lg border border-red-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Datang tapi tidak scan, atau tidak hadir
                            </p>
                            <p className="text-xs text-red-700 mt-1 italic">
                  ...
                  {/* TABEL 3: Evaluasi Kehadiran */}
                  <div className="mb-6 bg-white p-4 md:p-6 rounded-lg shadow-sm border-l-4 border-purple-500">
                    <h5 className="font-bold text-lg md:text-xl text-purple-700 mb-3">
                      3️⃣ Evaluasi Kehadiran (Status Resmi)
                    </h5>
                    <p className="text-sm md:text-base text-gray-700 mb-5">
                      Status kehadiran berdasarkan jadwal mengajar dan ketepatan waktu
                    </p>
                    ...
                    <div className="space-y-4 md:grid md:grid-cols-2 md:gap-4">
                      ...
                      <div className="bg-green-50 p-3 md:p-4 rounded-lg border border-green-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-green-300 rounded-lg flex items-center justify-center text-3xl font-bold text-green-900">
                              H
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Scan <strong>SEBELUM/TEPAT</strong> jadwal mengajar dimulai
                            </p>
                            <p className="text-xs text-green-700 mt-1 italic">
                              ✅ Status sempurna
                            </p>
                      ...
                      <div className="bg-yellow-50 p-3 md:p-4 rounded-lg border border-yellow-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-yellow-300 rounded-lg flex items-center justify-center text-3xl font-bold text-yellow-900">
                              H
                            </div>
                          </div>
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Scan <strong>SETELAH</strong> jadwal mengajar dimulai
                            </p>
                            <p className="text-xs text-yellow-700 mt-1 italic">
                              ⚠️ Status terlambat
                            </p>
                      ...
                      <div className="bg-red-50 p-3 md:p-4 rounded-lg border border-red-200">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          <div className="flex-shrink-0">
                            <div className="w-20 h-16 bg-red-300 rounded-lg flex items-center justify-center text-4xl font-bold text-red-900">
                              -
                            </div>
                          </div>
                          <div className="flex-1">
                      ...
                      <div className="bg-gray-50 p-3 md:p-4 rounded-lg border border-gray-300">
                        <div className="flex flex-col sm:flex-row gap-3 items-start">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-xs md:text-sm text-gray-700 mt-1">
                              Tidak ada <strong>JADWAL MENGAJAR</strong> (OFF/Jumat)
                            </p>
                            <p className="text-xs text-gray-600 mt-1 italic">
                              📅 Status hari libur
                            </p>
                  ...
                  {/* Tips Membaca */}
                  <div className="mb-6 p-4 md:p-5 bg-gradient-to-r from-indigo-100 to-purple-100 rounded-lg border border-indigo-300">
                    <h6 className="font-bold text-base md:text-lg text-indigo-800 mb-3">
                      💡 Tips Membaca:
                    </h6>
                    <ul className="text-xs md:text-sm text-gray-700 space-y-2">
                      <li className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2">
                        <span className="font-bold text-indigo-600 sm:min-w-[60px]">
                          Tabel 1:
                        </span>
                        <span>Cek kehadiran mentah (ada scan atau tidak)</span>
                      </li>
                      <li className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2">
                        <span className="font-bold text-indigo-600 sm:min-w-[60px]">
                          Tabel 2:
                        </span>
                        <span>Evaluasi kedisiplinan (tepat waktu atau lupa scan)</span>
                      </li>
                      <li className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2">
                        <span className="font-bold text-indigo-600 sm:min-w-[60px]">
                          Tabel 3:
                        </span>
                        <span>Status resmi kehadiran (H/L/-)</span>
                      </li>
                      <li className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2">
                        <span className="font-bold text-indigo-600 sm:min-w-[60px]">
                          Kolom Kanan:
                        </span>
                        <span>Total perhitungan & persentase</span>
                      </li>
                    </ul>
                  </div>
                </div>
